...
void Vvod(Man*& person, int num, int k) // ввод для первого массива
{
...
void deleted(Man*& person, int& num) // Удаление всех эл. с указанным ростом и весом
{
	system("chcp 1251>nul");
	int height1, weight1;
	cout << "Введите рост. ";
...
void Vvod1(Man*& person0, int num, int k) // ввод для второго массива
{
	system("chcp 1251>nul");
	int  YOB1 = -1, growth1 = -1, weight1 = -1;
	for (int i = k; i < num; i++)
...
void added(Man*& person, Man*& person0, int& num) // добавление элемента после указанного эл.
{
	bool k = false;
	string surname1;
